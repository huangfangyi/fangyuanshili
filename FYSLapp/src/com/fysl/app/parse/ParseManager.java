...
public class ParseManager {
	...
	public void getContactInfos(List<String> usernames,
			final EMValueCallBack<List<EaseUser>> callback) {
		ParseQuery<ParseObject> pQuery = ParseQuery.getQuery(CONFIG_TABLE_NAME);
			...
			public void done(List<ParseObject> arg0, ParseException arg1) {
				if (arg0 != null) {
					...
					for (ParseObject pObject : arg0) {
						EaseUser user = new EaseUser(pObject
								.getString(CONFIG_USERNAME));
						ParseFile parseFile = pObject
								.getParseFile(CONFIG_AVATAR);
						if (parseFile != null) {
		...
		pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
			...
			public void done(ParseObject pUser, ParseException e) {
				if (pUser != null) {
					...
					if (callback != null) {
						EaseUser user = DemoHelper.getInstance()

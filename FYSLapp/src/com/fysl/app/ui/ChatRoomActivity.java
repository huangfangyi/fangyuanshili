...
import com.fysl.app.adapter.ChatRoomAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		chatListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (position == 1) {
					// 添加公开群
					startActivityForResult(new Intent(ChatRoomActivity.this,
							PublicChatRoomsActivity.class), 0);
				} else {
					// 进入群聊
					...
					intent.putExtra("userId",
							chatRoomAdapter.getItem(position - 2).getId());
		...
		chatListView.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
					int position, long id) {
				if (position > 1) {
					final String roomId = chatRoomAdapter.getItem(position - 2)
							.getId();

...
public class ChatRoomDetailsActivity extends BaseActivity implements
		OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		chatRoomIdTextView.setText(getResources().getString(R.string.chat_room)
				+ " ID：" + roomId);
	...
	public void clearGroupHistory() {
		EMConversation conversation = EMClient.getInstance().chatManager()
				.getConversation(room.getId(), EMConversationType.ChatRoom);
	...
	protected void updateRoom() {
		new Thread(new Runnable() {
			public void run() {
				try {
					final EMChatRoom returnRoom = EMClient.getInstance()
							.chatroomManager().fetchChatRoomFromServer(roomId);

					runOnUiThread(new Runnable() {
						public void run() {
							((TextView) findViewById(R.id.group_name))
									.setText(returnRoom.getName());
							loadingPB.setVisibility(View.INVISIBLE);
							adapter.notifyDataSetChanged();
							if (EMClient.getInstance().getCurrentUser()
									.equals(returnRoom.getOwner())) {
								// 显示解散按钮
								exitBtn.setVisibility(View.GONE);
								deleteBtn.setVisibility(View.GONE);
							} else {
	...
	public void onClick(View v) {
		...
		case R.id.clear_all_history: // 清空聊天记录
			...
			new EaseAlertDialog(ChatRoomDetailsActivity.this, null, st9, null,
					new AlertDialogUser() {
				...
				if (!room.getOwner().equals(
					...
					button.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							// 进入选人页面
							// startActivityForResult(
							// (new Intent(ChatRoomDetailsActivity.this,
					...
					public void onClick(View v) {
						if (isInDeleteMode) {
							...
							if (EMClient.getInstance().getCurrentUser()
								...
								new EaseAlertDialog(
										ChatRoomDetailsActivity.this,
										R.string.not_delete_myself).show();
								return;
							}

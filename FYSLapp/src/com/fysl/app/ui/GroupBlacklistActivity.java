...
public class GroupBlacklistActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					List<String> blockedList = EMClient.getInstance()
							.groupManager().getBlockedUsers(groupId);
					if (blockedList != null) {
						Collections.sort(blockedList);
						adapter = new BlacklistAdapter(
								GroupBlacklistActivity.this, 1, blockedList);
					...
					runOnUiThread(new Runnable() {
						public void run() {
							Toast.makeText(getApplicationContext(), st1, 1)
									.show();
							progressBar.setVisibility(View.INVISIBLE);
						}
					});
	...
	void removeOutBlacklist(final String tobeRemoveUser) {
		...
		try {
			...
			EMClient.getInstance().groupManager()
					.unblockUser(groupId, tobeRemoveUser);

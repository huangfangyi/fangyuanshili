		...
		publibCheckBox
				.setOnCheckedChangeListener(new OnCheckedChangeListener() {
					...
					public void onCheckedChanged(CompoundButton buttonView,
						...
						if (isChecked) {
							secondTextView
									.setText(R.string.join_need_owner_approval);
						} else {
							secondTextView
									.setText(R.string.Open_group_members_invited);
						}
					}
				});
	}
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		if (resultCode == RESULT_OK) {
			...
			new Thread(new Runnable() {
				...
				public void run() {
					// 调用sdk创建群组方法
					final String groupName = groupNameEditText.getText()
							.toString().trim();
					...
					try {
						...
						String reason = NewGroupActivity.this
								.getString(R.string.invite_join_group);
						reason = EMClient.getInstance().getCurrentUser()
								+ reason + groupName;

						if (publibCheckBox.isChecked()) {
							option.style = memberCheckbox.isChecked() ? EMGroupStyle.EMGroupStylePublicJoinNeedApproval
									: EMGroupStyle.EMGroupStylePublicOpenJoin;
						} else {
							option.style = memberCheckbox.isChecked() ? EMGroupStyle.EMGroupStylePrivateMemberCanInvite
